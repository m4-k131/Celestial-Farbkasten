...
def test_main_fuzzy_search_calls_correctly(mock_astroquery, tmp_path):
    ...
    target = "NGC-628"
    with patch("src.query_observation_lists.os.getenv", return_value="fake-token"), patch("src.query_observation_lists.os.makedirs") as mock_mkdir, patch("pandas.DataFrame.to_csv") as mock_to_csv:
        query_main(target_name=target, exact_name=False, outdir=test_outdir)
        # --- Assertions ---
