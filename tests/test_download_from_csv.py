...
def mock_csv_data():
    ...
    dummy_data = {
        'intentType': ['science', 'science', 'calibration', 'science'],
        'jpegURL': ['mast:JWST/jpg/jw01.jpg', 'mast:JWST/jpg/jw02-filter.jpg', 'mast:JWST/jpg/cal.jpg', pd.NA],
        'dataURL': ['mast:JWST/fits/jw01.fits', 'mast:JWST/fits/jw02-filter.fits', 'mast:JWST/fits/cal.fits', 'mast:JWST/fits/jw03.fits']
    }
...
def mock_dependencies(mock_csv_data):
    ...
    with patch('src.download_from_csv.pd.read_csv', return_value=mock_csv_data) as mock_read_csv, \
        patch('src.download_from_csv.os.makedirs') as mock_mkdir, \
        patch('src.download_from_csv.os.path.isfile', return_value=False) as mock_isfile, \
        patch('src.download_from_csv.download_with_tqdm', return_value=True) as mock_download:
...
def test_download_all(mock_dependencies, tmp_path):
    ...
    expected_calls = [
        call(f"{BASE_URL}mast:JWST/jpg/jw01.jpg", os.path.join(test_outdir, 'jw01.jpg')),
        call(f"{BASE_URL}mast:JWST/fits/jw01.fits", os.path.join(test_outdir, 'jw01.fits')),
        call(f"{BASE_URL}mast:JWST/jpg/jw02-filter.jpg", os.path.join(test_outdir, 'jw02-filter.jpg')),
        call(f"{BASE_URL}mast:JWST/fits/jw02-filter.fits", os.path.join(test_outdir, 'jw02-filter.fits')),
        call(f"{BASE_URL}mast:JWST/fits/jw03.fits", os.path.join(test_outdir, 'jw03.fits'))
    ]
